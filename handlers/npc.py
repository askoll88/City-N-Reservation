...
def _forester_pale_watcher_note(user_id: int) -> str | None:
    ...
    if omen_count == 1:
        return (
            ...
            "«Не мерь. И не стой над ним долго. Если рядом нет второго отпечатка — это уже не след зверя.»"
        )
    return None
...
def handle_forester_trial_choice(player, vk, user_id: int, text: str, stage: str):
    ...
    choices = {
        ...
        "exit": {
            ...
            "good_note": "«После контакта главное — обратная дорога. Метки спасают, когда в панике начинаешь путать тропу.»",
...
def _handle_soldier_shells_bag_info(player, vk, user_id: int, npc_id: str):
    ...
    lines = [
        "🎖️Военный:\n",
        "Базовый мешочек на 10 гильз теперь входит в набор новичка.",
        "У меня берут только апгрейды, когда старого объема уже мало.",
    ...
    if highest_owned_index >= len(bag_order) - 1:
        ...
        lines.extend([
            ...
            "Апгрейды идут от раннего допуска до 120 уровня.",
        ])
