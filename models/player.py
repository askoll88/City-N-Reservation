...
def get_radiation_stage(radiation: int) -> dict:
    ...
    rad = max(0, int(radiation or 0))
    if rad < 30:
        return {"name": "0. Фоновый уровень", "note": "Клинических симптомов нет"}
    if rad < 80:
        return {"name": "I. Лёгкое облучение", "note": "Риск ранних симптомов"}
    if rad < 120:
        return {"name": "II. Ранняя лучевая реакция", "note": "Слабость, тошнота, падение выносливости"}
    if rad < 170:
        return {"name": "III. Средняя лучевая болезнь", "note": "Стабильный вред без лечения"}
    if rad < 220:
        return {"name": "IV. Тяжёлая лучевая болезнь", "note": "Высокий риск отказа организма"}
    if rad < 250:
        return {"name": "V. Критическое состояние", "note": "Нужен антирад немедленно"}
    return {"name": "VI. Терминальная стадия", "note": "Почти неизбежная смерть без срочной помощи"}
